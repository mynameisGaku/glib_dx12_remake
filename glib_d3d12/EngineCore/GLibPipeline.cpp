...
bool glib::GLibPipeline::Initialize(GLibDevice* device, const D3D12_GRAPHICS_PIPELINE_STATE_DESC& psoDesc)
{
    m_Hr = device->Get()->CreateGraphicsPipelineState(&psoDesc, IID_PPV_ARGS(&m_PipelineState));
    if (FAILED(m_Hr))
    {
