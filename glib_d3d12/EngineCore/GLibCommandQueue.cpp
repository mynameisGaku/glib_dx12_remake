...
bool glib::GLibCommandQueue::Initialize(GLibDevice* device, const D3D12_COMMAND_QUEUE_DESC& desc)
{
    m_Hr = device->Get()->CreateCommandQueue(&desc, IID_PPV_ARGS(m_CommandQueue.GetAddressOf()));
    if (FAILED(m_Hr))
    {
